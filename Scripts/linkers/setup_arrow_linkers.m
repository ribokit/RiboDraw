...
for i = 1:length(resnum)
    ...
    res_tag_i = sprintf('Residue_%s%s%d',chains(i),segid{i},resnum(i));
    res_tag_j = sprintf('Residue_%s%s%d',chains(j),segid{j},resnum(j));
    linker.residue1 = res_tag_i;
    linker.residue2 = res_tag_j;
    linker.type = 'arrow';
    linker_tag = sprintf('Linker_%s%s%d_%s%s%d_%s', chains(i),segid{i},resnum(i),chains(j),segid{j},resnum(j),linker.type);
